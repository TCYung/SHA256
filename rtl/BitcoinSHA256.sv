//difference between this file and the ControlTestSV file is that Bitcoin swaps the bits around for the merkle root and previous hash before putting it through the SHA256 algorithm
//it also uses little endian (smallest bit first) compared to the big endian that I am more familiar with
//little endian is used for the version, timestamp, difficulty, and nonce
//after the 640 bit input is converted to 256 bits, the 256 bit output is put through the SHA256 algorithm again (hash = SHA256(SHA256(Block_Header)))
//the hash output needs to be swapped to show the leading 0 format 

//ex of swapping (left hand side is what is inputted into the SHA algorithm)
//81cd02ab7e569e8bcd9317e2fe99f2de44d49ab2b8851ba4a308000000000000 = 00000000000008a3a41b85b8b29ad444def299fee21793cd8b9e567eab02cd81
//e320b6c2fffc8d750423db8b1eb942ae710e951ed797f7affc8892b0f1fc122b = 2b12fcf1b09288fcaff797d71e950e71ae42b91e8bdb2304758dfcffc2b620e3

module Bitcoin_SHA256
(   input clk,
    input rst,

    input second_hash, //0 means that its a 640 bit input, 1 means 256 bit input
    input [255:0] input_hash, //isnt used on the 640 bit input, used on the 256 bit input (hash1 is the input_hash)

    input [255:0] hash_target, //the resultant hash needs to be less than this value

    input [31:0] nonce, //from top module this value gets changed every time the end hash is not correct

    input hash_status,
    output reg hash_complete,
    output reg [255:0] hash = 256'd0
);

reg [31:0] h0 = 32'h6a09e667;
reg [31:0] h1 = 32'hbb67ae85;
reg [31:0] h2 = 32'h3c6ef372;
reg [31:0] h3 = 32'ha54ff53a;
reg [31:0] h4 = 32'h510e527f;
reg [31:0] h5 = 32'h9b05688c;
reg [31:0] h6 = 32'h1f83d9ab;
reg [31:0] h7 = 32'h5be0cd19;
reg [31:0] k [0:63];

//for this test scenario we are going to be using bitcoin block 923948
//target hash = 000000000000000000001f276a92e92679e23e610e3392d7d15b1684089af45c

reg [31:0] bitcoin_version =  {<<byte {32'h3fff0000}};
reg [255:0] previous_hash = {<<byte {256'h00000000000000000000a94278b1c645a52dfc46bf6f985010f5626093b0eb9f}}; //this is the hash of block 923947
reg [255:0] merkle_root = {<<byte {256'h8d6f9f82908f4e916af2eb57010de762f46ea5923374898033b0599acf6c8bc0}};
reg [31:0] timestamp = {<<byte {32'h691A4FD1}}; //1763332049 or 11/16/25 5:27:29 pm 
reg [31:0] difficulty = {<<byte {32'h1701d936}};

//reg [31:0] nonce = {<<byte {32'h7481a217}}; // 1954652695 in decimal

reg [63:0] length_bits = 64'd640; //640 is the length of the bitcoin block header

reg [511:0] chunk1;
reg [511:0] chunk2;

reg [31:0] a_temp, b_temp, c_temp, d_temp, e_temp, f_temp, g_temp, h_temp;

initial begin
   k[0] = 32'h428a2f98;
   k[1] = 32'h71374491;
   k[2] = 32'hb5c0fbcf;
   k[3] = 32'he9b5dba5;
   k[4] = 32'h3956c25b;
   k[5] = 32'h59f111f1;
   k[6] = 32'h923f82a4;
   k[7] = 32'hab1c5ed5;
   k[8] = 32'hd807aa98;
   k[9] = 32'h12835b01;
   k[10] = 32'h243185be;
   k[11] = 32'h550c7dc3;
   k[12] = 32'h72be5d74;
   k[13] = 32'h80deb1fe;
   k[14] = 32'h9bdc06a7;
   k[15] = 32'hc19bf174;
   k[16] = 32'he49b69c1;
   k[17] = 32'hefbe4786;
   k[18] = 32'h0fc19dc6;
   k[19] = 32'h240ca1cc;
   k[20] = 32'h2de92c6f;
   k[21] = 32'h4a7484aa;
   k[22] = 32'h5cb0a9dc;
   k[23] = 32'h76f988da;
   k[24] = 32'h983e5152;
   k[25] = 32'ha831c66d;
   k[26] = 32'hb00327c8;
   k[27] = 32'hbf597fc7;
   k[28] = 32'hc6e00bf3;
   k[29] = 32'hd5a79147;
   k[30] = 32'h06ca6351;
   k[31] = 32'h14292967;
   k[32] = 32'h27b70a85;
   k[33] = 32'h2e1b2138;
   k[34] = 32'h4d2c6dfc;
   k[35] = 32'h53380d13;
   k[36] = 32'h650a7354;
   k[37] = 32'h766a0abb;
   k[38] = 32'h81c2c92e;
   k[39] = 32'h92722c85;
   k[40] = 32'ha2bfe8a1;
   k[41] = 32'ha81a664b;
   k[42] = 32'hc24b8b70;
   k[43] = 32'hc76c51a3;
   k[44] = 32'hd192e819;
   k[45] = 32'hd6990624;
   k[46] = 32'hf40e3585;
   k[47] = 32'h106aa070;
   k[48] = 32'h19a4c116;
   k[49] = 32'h1e376c08;
   k[50] = 32'h2748774c;
   k[51] = 32'h34b0bcb5;
   k[52] = 32'h391c0cb3;
   k[53] = 32'h4ed8aa4a;
   k[54] = 32'h5b9cca4f;
   k[55] = 32'h682e6ff3;
   k[56] = 32'h748f82ee;
   k[57] = 32'h78a5636f;
   k[58] = 32'h84c87814;
   k[59] = 32'h8cc70208;
   k[60] = 32'h90befffa;
   k[61] = 32'ha4506ceb;
   k[62] = 32'hbef9a3f7;
   k[63] = 32'hc67178f2;
   
end
reg chunk_flag = 0; //if 0 we are on chunk 1, if 1 we are on chunk 2

reg [31:0] w_chunk1 [0:63];
reg [31:0] w_chunk2 [0:63];

reg [8:0] main_loop_counter = 0;
reg [8:0] extend_counter = 16;

//for chunk 1
reg [31:0] s0_1; 
reg [31:0] s1_1;

//for chunk 2
reg [31:0] s0_2;
reg [31:0] s1_2;

reg [31:0] s1, ch, temp1, s0, maj, temp2; //temporary variables named according to sha2 wiki main loop

reg [7:0] state;
parameter idle_state = 8'b0000001; 
parameter w_chunk_initial = 8'b0000010; 
parameter w_chunk_extend = 8'b0000011;
parameter main_loop_chunks = 8'b0000100;
parameter main_loop_complete = 8'b0000110;

reg [7:0] main_loop_state;
parameter main_loop_variables = 8'b0000111;
parameter main_loop_temp = 8'b0001000;
parameter main_loop_assign = 8'b0001001;

always_comb begin
    //w_chunk_extend chunk 1 and chunk 2
    s0_1 = ({w_chunk1[extend_counter-15][6:0], w_chunk1[extend_counter-15][31:7]}) ^ ({w_chunk1[extend_counter-15][17:0], w_chunk1[extend_counter-15][31:18]}) ^ (w_chunk1[extend_counter-15] >> 3); //right rotate 7, right rotate 18, right shift 3
    s1_1 = ({w_chunk1[extend_counter-2][16:0], w_chunk1[extend_counter-2][31:17]}) ^ ({w_chunk1[extend_counter-2][18:0], w_chunk1[extend_counter-2][31:19]}) ^ (w_chunk1[extend_counter-2] >> 10); //right rotate 17, right rotate 19, right shift 10

    //second pass only has 256 bits which only needs 1 chunk which means the below code doesnt need to be executed 
    if (!second_hash) begin
        s0_2 = ({w_chunk2[extend_counter-15][6:0], w_chunk2[extend_counter-15][31:7]}) ^ ({w_chunk2[extend_counter-15][17:0], w_chunk2[extend_counter-15][31:18]}) ^ (w_chunk2[extend_counter-15] >> 3); //right rotate 7, right rotate 18, right shift 3
        s1_2 = ({w_chunk2[extend_counter-2][16:0], w_chunk2[extend_counter-2][31:17]}) ^ ({w_chunk2[extend_counter-2][18:0], w_chunk2[extend_counter-2][31:19]}) ^ (w_chunk2[extend_counter-2] >> 10); //right rotate 17, right rotate 19, right shift 10
    end

    else begin
        s0_2 = 1'bX;
        s1_2 = 1'bX;
    end

end

always_ff @(posedge clk) begin 

    //reset for when the nonce is being incremented 
    if (rst) begin
        h0 = 32'h6a09e667;
        h1 = 32'hbb67ae85;
        h2 = 32'h3c6ef372;
        h3 = 32'ha54ff53a;
        h4 = 32'h510e527f;
        h5 = 32'h9b05688c;
        h6 = 32'h1f83d9ab;
        h7 = 32'h5be0cd19;
        
        hash_complete <= 0;
        state <= idle_state;
    end

    else begin
        case(state)	
            default: begin
            state <= idle_state;
            end
            
            idle_state: begin
                if (!second_hash) begin
                    chunk1 <= {bitcoin_version, previous_hash, merkle_root[255:32]};
                    chunk2 <= {merkle_root[31:0], timestamp, difficulty, nonce, 1'b1, 319'b0, length_bits};
                    state <= w_chunk_initial;
                end
                
                if (second_hash && hash_status == 1 && input_hash !== 0) begin //hash status means that the first hash has finished (dont want the second hash to begin without output from 1st hash)
                    chunk1 <= {input_hash, 1'b1, 191'b0, 64'd256};
                    state <= w_chunk_initial;
                end            
            end
                
            w_chunk_initial: begin
                w_chunk1[0] <= chunk1[511 - 0*32 : 480 - 0*32];		//[512 : 480]
                w_chunk1[1] <= chunk1[511 - 1*32 : 480 - 1*32];		//[479 : 448]
                w_chunk1[2] <= chunk1[511 - 2*32 : 480 - 2*32];		//[447 : 416]
                w_chunk1[3]  <= chunk1[511 - 3*32 : 480 - 3*32];	//[415 : 384]
                w_chunk1[4]  <= chunk1[511 - 4*32 : 480 - 4*32];	//[383 : 352]
                w_chunk1[5]  <= chunk1[511 - 5*32 : 480 - 5*32];	//[351 : 320]
                w_chunk1[6]  <= chunk1[511 - 6*32 : 480 - 6*32];	//[319 : 288]
                w_chunk1[7]  <= chunk1[511 - 7*32 : 480 - 7*32];	//[287 : 256]
                w_chunk1[8]  <= chunk1[511 - 8*32 : 480 - 8*32];	//[255 : 224]
                w_chunk1[9]  <= chunk1[511 - 9*32 : 480 - 9*32];	//[223 : 192]
                w_chunk1[10] <= chunk1[511 - 10*32: 480 - 10*32];	//[191 : 160]
                w_chunk1[11] <= chunk1[511 - 11*32: 480 - 11*32];	//[159 : 128]
                w_chunk1[12] <= chunk1[511 - 12*32: 480 - 12*32];	//[127 : 96]
                w_chunk1[13] <= chunk1[511 - 13*32: 480 - 13*32];	//[95  : 64]
                w_chunk1[14] <= chunk1[511 - 14*32: 480 - 14*32];	//[63  : 32]
                w_chunk1[15] <= chunk1[511 - 15*32: 480 - 15*32];	//[31  : 0]

                if (!second_hash) begin
                    w_chunk2[0] <= chunk2[511 - 0*32 : 480 - 0*32];		//[512 : 480]
                    w_chunk2[1]  <= chunk2[511 - 1*32 : 480 - 1*32];	//[479 : 448]
                    w_chunk2[2]  <= chunk2[511 - 2*32 : 480 - 2*32];	//[447 : 416]
                    w_chunk2[3]  <= chunk2[511 - 3*32 : 480 - 3*32];	//[415 : 384]
                    w_chunk2[4]  <= chunk2[511 - 4*32 : 480 - 4*32];	//[383 : 352]
                    w_chunk2[5]  <= chunk2[511 - 5*32 : 480 - 5*32];	//[351 : 320]
                    w_chunk2[6]  <= chunk2[511 - 6*32 : 480 - 6*32];	//[319 : 288]
                    w_chunk2[7]  <= chunk2[511 - 7*32 : 480 - 7*32];	//[287 : 256]
                    w_chunk2[8]  <= chunk2[511 - 8*32 : 480 - 8*32];	//[255 : 224]
                    w_chunk2[9]  <= chunk2[511 - 9*32 : 480 - 9*32];	//[223 : 192]
                    w_chunk2[10] <= chunk2[511 - 10*32: 480 - 10*32];	//[191 : 160]
                    w_chunk2[11] <= chunk2[511 - 11*32: 480 - 11*32];	//[159 : 128]
                    w_chunk2[12] <= chunk2[511 - 12*32: 480 - 12*32];	//[127 : 96]
                    w_chunk2[13] <= chunk2[511 - 13*32: 480 - 13*32];	//[95  : 64]
                    w_chunk2[14] <= chunk2[511 - 14*32: 480 - 14*32];	//[63  : 32]
                    w_chunk2[15] <= chunk2[511 - 15*32: 480 - 15*32];	//[31  : 0]
                end
                
                state <= w_chunk_extend;
            end

            w_chunk_extend: begin            	               
                if (extend_counter !== 64) begin 
                    extend_counter <= extend_counter + 1;
                    
                    //chunk 1                
                    w_chunk1[extend_counter] <= w_chunk1[extend_counter-16] + s0_1 + w_chunk1[extend_counter-7] + s1_1; 
                    
                    //chunk 2
                    if (!second_hash) begin
                        w_chunk2[extend_counter] <= w_chunk2[extend_counter-16] + s0_2 + w_chunk2[extend_counter-7] + s1_2;
                    end

                    state <= w_chunk_extend;                    
                end

                //this would save a clock cycle since putting it in the main_loop_counter section would mean you need to wait a cycle
                if (extend_counter == 64) begin 
                    a_temp <= h0;
                    b_temp <= h1;
                    c_temp <= h2;
                    d_temp <= h3;
                    e_temp <= h4;
                    f_temp <= h5;
                    g_temp <= h6;
                    h_temp <= h7;
                    
                    extend_counter <= 16;
                    state <= main_loop_chunks;
                end
            end

            //grouped chunk 1 and chunk 2 into 1 case statement to reduce the amount of code
            main_loop_chunks: begin
                if (main_loop_counter !== 64 && main_loop_counter !== 65) begin //splitting this into 3 solved my issues 
                    case (main_loop_state)
                        default: begin
                            main_loop_state <= main_loop_variables;
                        end

                        main_loop_variables: begin //assigns variables that are used in the next 2 sections of code
                            s1 <= ({e_temp[5:0], e_temp[31:6]}) ^ ({e_temp[10:0], e_temp[31:11]}) ^ ({e_temp[24:0], e_temp[31:25]}); //right rotate 6, right rotate 11, right rotate 25
                            ch <= (e_temp & f_temp) ^ (~e_temp & g_temp);

                            s0 <= ({a_temp[1:0], a_temp[31:2]}) ^ ({a_temp[12:0], a_temp[31:13]}) ^ ({a_temp[21:0], a_temp[31:22]}); //right rotate 2, right rotate 13, right rotate 22
                            maj <= (a_temp & b_temp) ^ (a_temp & c_temp) ^ (b_temp & c_temp);

                            main_loop_state <= main_loop_temp;
                        end

                        main_loop_temp: begin //assigns temporary variables
                            if (!chunk_flag) begin //chunk flag is only 0 when i am processing the first chunk
                                temp1 <= h_temp + s1 + ch + k[main_loop_counter] + w_chunk1[main_loop_counter]; //only difference between chunk 1 and 2 should be the w variable                
                            end
                            
                            else begin
                                temp1 <= h_temp + s1 + ch + k[main_loop_counter] + w_chunk2[main_loop_counter]; //only difference between chunk 1 and 2 should be the w variable                
                            end
                            
                            temp2 <= s0 + maj;	                           
                            main_loop_state <= main_loop_assign;
                        end
                        
                        main_loop_assign: begin //assign the final values 
                            h_temp <= g_temp;
                            g_temp <= f_temp;
                            f_temp <= e_temp;
                            e_temp <= d_temp + temp1;
                            d_temp <= c_temp;
                            c_temp <= b_temp;
                            b_temp <= a_temp;
                            a_temp <= temp1 + temp2;

                            main_loop_counter <= main_loop_counter + 1;
                            main_loop_state <= main_loop_variables;
                        end
                    endcase
                end
                
                //there is probably a better way to write the below code 
                //maybe another state? once the first chunk is done i set chunk_flag and reset the counter to 0 and main_loop_temp uses chunk2's temporary values

                if (main_loop_counter == 64) begin
                    h0 <= h0 + a_temp;
                    h1 <= h1 + b_temp;
                    h2 <= h2 + c_temp;
                    h3 <= h3 + d_temp;
                    h4 <= h4 + e_temp;
                    h5 <= h5 + f_temp;
                    h6 <= h6 + g_temp;
                    h7 <= h7 + h_temp;

                    main_loop_counter <= main_loop_counter + 1;                 
                    
                    if (chunk_flag || second_hash) begin //chunk_flag is when 2 chunks have been processed already (640 bit hash1) ; second_hash is if there is only 1 chunk (256 bit hash2)
                        main_loop_counter <= 0;
                        state <= main_loop_complete;
                    end
                end

                if (main_loop_counter == 65) begin 
                    a_temp <= h0;
                    b_temp <= h1;
                    c_temp <= h2;
                    d_temp <= h3;
                    e_temp <= h4;
                    f_temp <= h5;
                    g_temp <= h6;
                    h_temp <= h7;

                    chunk_flag <= 1; 

                    main_loop_counter <= 0;                    
                end
            end
            
            main_loop_complete: begin
                hash <= {h0, h1, h2, h3, h4, h5, h6, h7}; //note that this hash output is not formatted properly so you need to do the (<<byte {) operation if you want to check if this value is less than the target hash 
                hash_complete <= 1;
                chunk_flag <= 0;

            end
            endcase
        end
	end
endmodule
